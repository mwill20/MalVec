# MalVec Lessons

> **Philosophy:** Lessons are created alongside code, not as an afterthought.  
> Each phase produces both working code AND teaching material.

---

## 📚 Lesson Tracks

### Novice Track (16 Lessons)

*Target: Beginners to malware analysis, basic Python knowledge*

| # | Lesson | Phase | Status |
|---|--------|-------|--------|
| 01 | [Project Foundation](novice/lesson_01_project_foundation.md) | 1 | ✅ Complete |
| 02 | [EMBER Dataset Integration](novice/lesson_02_ember_integration.md) | 2 | ✅ Complete |
| 03 | Feature Vector Exploration | 2 | ⏳ Planned |
| 04 | Embedding Generation | 3 | ⏳ Planned |
| 05 | Semantic Similarity | 3 | ⏳ Planned |
| 06 | Vector Database Basics | 4 | ⏳ Planned |
| 07 | K-NN Search | 4 | ⏳ Planned |
| 08 | Classification Logic | 5 | ⏳ Planned |
| 09 | Confidence Scoring | 5 | ⏳ Planned |
| 10 | CLI Development | 6 | ⏳ Planned |
| 11 | Training Pipeline | 6 | ⏳ Planned |
| 12 | Input Validation | 7 | ⏳ Planned |
| 13 | Static Analysis | 7 | ⏳ Planned |
| 14 | Defense in Depth | 8 | ⏳ Planned |
| 15 | Testing Strategies | 9 | ⏳ Planned |
| 16 | Deployment Basics | 9 | ⏳ Planned |

### Professional Track (22 Lessons)

*Target: Working security engineers, production deployment*

| # | Lesson | Phase | Status |
|---|--------|-------|--------|
| 01 | [Production Architecture](professional/lesson_01_production_architecture.md) | 1 | ✅ Complete |
| 02 | [EMBER Integration & Feature Engineering](professional/lesson_02_ember_feature_engineering.md) | 2 | ✅ Complete |
| 03 | Feature Pipeline Design | 2 | ⏳ Planned |
| 04 | Embedding Model Selection | 3 | ⏳ Planned |
| 05 | Embedding Optimization | 3 | ⏳ Planned |
| 06 | Vector Storage Architecture | 4 | ⏳ Planned |
| 07 | FAISS at Scale | 4 | ⏳ Planned |
| 08 | Classification Algorithms | 5 | ⏳ Planned |
| 09 | Confidence Calibration | 5 | ⏳ Planned |
| 10 | CLI UX Design | 6 | ⏳ Planned |
| 11 | Training Pipeline Optimization | 6 | ⏳ Planned |
| 12 | Input Validation Hardening | 7 | ⏳ Planned |
| 13 | PE/ELF Analysis | 7 | ⏳ Planned |
| 14 | Sandbox Architecture | 8 | ⏳ Planned |
| 15 | Defense in Depth | 8 | ⏳ Planned |
| 16 | Unit Testing Security Tools | 9 | ⏳ Planned |
| 17 | Integration Testing | 9 | ⏳ Planned |
| 18 | E2E Testing | 9 | ⏳ Planned |
| 19 | Performance Tuning | 9 | ⏳ Planned |
| 20 | Deployment Patterns | 9 | ⏳ Planned |
| 21 | Monitoring & Alerting | 9 | ⏳ Planned |
| 22 | Incident Response | 9 | ⏳ Planned |

---

## 📖 Lesson Format

Each lesson includes:

1. **Learning Objectives** - What you'll know/do by the end
2. **Key Concepts** - Theory and principles
3. **Hands-On Exercises** - Practical application
4. **Checkpoint Questions** - Self-assessment
5. **Key Takeaways** - Summary principles
6. **Further Reading** - Deep dive resources

---

## 🎯 Track Differences

| Aspect | Novice | Professional |
|--------|--------|--------------|
| Assumed knowledge | Basic Python | Package development |
| Focus | Understanding concepts | Production patterns |
| Exercises | Guided implementation | Open-ended design |
| Depth | Breadth-first | Depth-first |
| Time per lesson | 45-60 min | 60-90 min |

---

*Lessons created alongside Phase builds. Last updated: 2026-02-01*
